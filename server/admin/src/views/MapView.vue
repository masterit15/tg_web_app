      ...
      <div class="catalog_list_item vert" v-for="item in items" :key="item.id">
        ...
        <div class="catalog_list_item_content">
          ...
          <div class="catalog_list_item_footer">
            <h2 class="catalog_list_item_price">
              <i class="fa fa-rub"></i> {{ item.price }}
            </h2>
            <span class="catalog_list_item_weight">{{
              calcWeight(item.weight)
            }}</span>
            <div class="catalog_list_item_action">
              <span
                class="catalog_list_item_action_bay minus"
                @click="bayCount($event, 'minus', item)"
                ><span></span
              ></span>
              <input
                type="text"
              ...
              />
              <span
                class="catalog_list_item_action_bay plus"
                @click="bayCount($event, 'plus', item)"
                ><span></span
              ></span>
            </div>
          </div>
  ...
  computed: {
    ...
    balloonTemplate() {
      return `
        ...
        <p>Я тут: ${this.coords}</p>
        <pre>sdfdsdfsfsdf</pre>
      `;
    },
  },
  methods: {
